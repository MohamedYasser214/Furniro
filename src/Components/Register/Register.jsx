...
export default function Register() {
  ...
  let validate = yup.object().shape({
    name:yup.string().min(3,'name minilength 3').max(10, 'name max length is 10').required('name is required'),
    email:yup.string().required('email is required').email('email is invalid'),
    phone:yup.string().matches(/^01[012][0-9]{8}$/ , 'phone must be valid egyption number').required('Phone is required'),
    password:yup.string().matches(/^[A-z][a-z0-9]{5,10}$/ , 'Password must Start with uppercase').required('password is required'),
    rePassword:yup.string().oneOf([yup.ref('password')],'Password is not match').required('password is required'),
...
return <>
  <div className='mx-auto py-6 max-w-xl my-15'>
    ...
    <h2 style={{color:'#e69430'}} className='text-3xl  font-bold mb-6'>Register Now</h2>
  ...
  {formik.errors.rePassword &&formik.touched.rePassword?<div className="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
    ...
    <button type="submit" className="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center  " style={{color:'white' ,backgroundColor:"#e69430"}}>
