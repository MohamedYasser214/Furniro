    ...
    <div className=" text-black">
      ...
      <div className={styles.logo}>
        <div className={styles.box}>
          <span className="text-yellow-600 font-semibold">New Arrival</span>
          <h1 className="text-4xl font-bold">Discover Our</h1>
      ...
      <div className="text-center my-10">
        ...
        <div className="my-10 grid grid-cols-2 md:grid-cols-3 gap-6">
          {[{ img: logo4, title: "Dining" }, { img: logo5, title: "Living" }, { img: logo6, title: "Bedroom" }].map((item, idx) => (
            <div key={idx} className="text-center">
              <img className="h-56 w-full object-cover rounded-lg" src={item.img} alt={item.title} />
              <h4 className="my-4 text-lg font-semibold">{item.title}</h4>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-6">
        <h1 className="text-center text-2xl font-bold my-4">Our Product</h1>
          ...
          {products.slice(0, visibleCount).map((product) => (
            ...
            >
              <Link to={`/product/${product.id}`}>
                <div className="bg-gray-100 flex items-center justify-center rounded h-48">
                  {product.image ? (
                    <img
                      src={product.image}
                      alt={product.name}
      ...
      <div
        ...
        className="flex flex-col md:flex-row items-center gap-6 p-10"
      >
